...
library(ggplot2)
library(dplyr)
recommendation <- read.csv('Prediction.csv',stringsAsFactors = F,header=T)
#head(recommendation)
...
frow2 <- fluidRow(
  ...
  box(
    title = "Forecasted_Generation.in.MW"
    ,status = "primary"
    ,solidHeader = TRUE 
    ,collapsible = TRUE 
  ...
  ,box(
    title = "Available_Capacity.AvC.MW"
    ,status = "primary"
    ,solidHeader = TRUE 
    ,collapsible = TRUE 
...
server <- function(input, output) { 
  ...
  recommendation$Forecasted_Generation.in.MW <- renderValueBox({
    valueBox(
